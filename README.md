...
- ✅ **语料数据管理**：增删改查，支持 Unicode 和 IPA 字符，支持分类和标签管理
- ✅ **IPA 音标工具栏**：可折叠的国际音标快速输入面板（元音、辅音、声调、变音符等）
- ✅ **搜索与筛选**：全文搜索、标签筛选、搜索结果高亮显示
- ✅ **数据导出**：支持导出为 Word 文档（词对齐格式）、CSV 文件、JSON 文件
- ✅ **AI 辅助功能**：集成 AI 词汇分解和智能翻译（混合模式架构）
- ✅ **多 LLM 支持**：Claude / OpenAI / DeepSeek / 通义千问 / 智谱GLM / 百度文心 / Ollama
- ✅ **界面主题**：支持深色模式和浅色模式切换
- ✅ **数据备份与恢复**：自动备份 + 手动恢复
- ✅ **打印功能**：格式化打印输出
- ✅ **数据完整性检查**：数据库校验功能
- ✅ **批量导入**：支持 JSON 和 CSV 格式的批量导入
- ✅ **批量标签管理**：同时为多条语料添加或移除标签
- ✅ **语料去重**：检测并处理重复语料记录
- ✅ **窗口记忆**：记住窗口位置和大小
...
- **数据存储**：SQLite3
- **文档导出**：python-docx
- **AI 接口**：Claude API / OpenAI 兼容协议 / Ollama
...
fieldnote/
├── main.py            # 主程序入口（单实例检查）
├── gui.py             # 图形界面模块（PyQt6 主窗口）
├── database.py        # 数据库模块（SQLite 封装）
...
### Q: 如何输入IPA字符？
A: 软件内置 IPA 音标工具栏，点击符号即可插入。也可以使用 IPA 输入法或直接复制粘贴。

### Q: 数据存储在哪里？
A: 数据存储在用户主目录 `~/.fieldnote/corpus.db` 中。

### Q: 如何备份数据？
A: 使用菜单"工具 > 备份数据库"进行手动备份，软件也会在关键操作前自动备份。也可以导出为 JSON 格式备份。
